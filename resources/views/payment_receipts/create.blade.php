...
@section('content')
    ...
    @else
        <div class="card" style="margin-bottom:16px;">
            <form method="GET" action="{{ route('payment-receipts.create') }}">
                <div class="form-grid">
        ...
        @if ($selectedBook === null)
            <div class="alert alert-error">入金を入力する帳簿を選択してください。</div>
        @elseif ($paymentSchedules->isEmpty())
            <div class="alert alert-error">この帳簿には入金予定がありません。先に入金予定を登録してください。</div>
        @else
            @if ($errors->any())
            ...
            <div class="card">
                <form method="POST" action="{{ route('payment-receipts.store') }}">
                    @csrf
                    ...
                    @include('payment_receipts._form')

                    <div class="actions" style="margin-top:24px;">
                        <button type="submit" class="button">登録する</button>
                        <a href="{{ route('payment-receipts.index', ['book_id' => $selectedBookId]) }}" class="button button-secondary">キャンセル</a>
                    </div>
                </form>
            </div>
        @endif
