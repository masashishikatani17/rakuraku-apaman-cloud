...
@section('content')
    ...
    <div class="page-header">
        <div>
            <h2 class="page-title">補助科目別元帳</h2>
            <p class="page-description">補助科目ごとの仕訳明細と残高を確認します。</p>
        </div>
        <div class="actions">
            @if ($selectedBookId)
                <a
                    href="{{ route('reports.sub-accounts.index', ['book_id' => $selectedBookId, 'date_from' => $dateFrom, 'date_to' => $dateTo]) }}"
    ...
    <div class="alert alert-success" style="background: #eff6ff; color: #1e3a8a; border-color: #bfdbfe;">
        初版では、選択した補助科目の仕訳明細を時系列で表示します。
        銀行口座別、取引先別、物件別などの内訳確認に使えます。
    </div>

    <div class="card" style="margin-bottom: 16px;">
        <form method="GET" action="{{ route('sub-account-ledgers.index') }}">
            <div class="form-grid">
                <div class="field">
                ...
                <div class="field">
                    <label for="sub_account_title_id">補助科目<span class="required">必須</span></label>
                    <select id="sub_account_title_id" name="sub_account_title_id" required>
                        @foreach ($subAccountTitles as $subAccountTitle)
                            ...
                            >
                                ...
                                /
                                {{ $subAccountTitle->sub_account_code }}
                                /
                                {{ $subAccountTitle->name }}
                            </option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="field">
                    ...
                    <div>
                        @if ($selectedSubAccountTitle)
                            {{ $selectedSubAccountTitle->sub_account_code }} / {{ $selectedSubAccountTitle->name }}
                        @else
                            —
                        @endif
                    </div>
                ...
                <div class="field">
                    ...
                    <div>
                        ...
                        @if ($summary['opening_balance_side'])
                            <span class="muted">
                                ({{ $sideLabels[$summary['opening_balance_side']] ?? $summary['opening_balance_side'] }})
                            </span>
                        @endif
                    </div>
                </div>
                ...
                <div class="field">
                    ...
                    <div>
                        ...
                        @if ($summary['ending_balance_side'])
                            <span class="muted">
                                ({{ $sideLabels[$summary['ending_balance_side']] ?? $summary['ending_balance_side'] }})
                            </span>
                        @endif
                    </div>
                </div>
    ...
    <div class="card">
        <table class="data-table">
            ...
            <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            @if ($row->running_balance_side)
                                <div class="muted">
                                    {{ $sideLabels[$row->running_balance_side] ?? $row->running_balance_side }}
                                </div>
                            @endif
                        </td>
                        <td>
                            ...
                            @if (!$row->department_code && !$row->department_name && !$row->line_note)
                                —
                            @endif
                        </td>
                        <td>
                ...
                @empty
                    <tr>
                        <td colspan="10">指定条件に一致する補助科目明細がありません。</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
