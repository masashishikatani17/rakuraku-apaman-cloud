...
@section('title', '入金予定修正')

@section('content')
    <div class="page-header">
        <div>
            <h2 class="page-title">入金予定修正</h2>
            <p class="page-description">登録済の入金予定を修正します。</p>
        </div>
        <div class="actions">
            <a href="{{ route('payment-schedules.index', ['book_id' => $selectedBookId]) }}" class="button button-secondary">入金予定一覧へ戻る</a>
    ...
    <div class="card">
        <form method="POST" action="{{ route('payment-schedules.update', $paymentSchedule) }}">
            @csrf
            @method('PUT')
