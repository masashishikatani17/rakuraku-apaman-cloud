...
{
    public function cashIndex(Request $request): View
    {
        return $this->index($request, 'cash');
    }

    public function bankIndex(Request $request): View
    {
        return $this->index($request, 'bank');
    }

    private function index(Request $request, string $ledgerType): View
    {
        ...
        if ($selectedBook !== null) {
            $accountTitles = $this->getSelectableAccountTitles((int) $selectedBook->id);

            if (
                $selectedAccountTitleId === null
                || !$accountTitles->contains('id', $selectedAccountTitleId)
            ) {
                $selectedAccountTitle = $this->guessDefaultAccountTitle($accountTitles, $ledgerType);
                $selectedAccountTitleId = $selectedAccountTitle?->id;
            } else {
                $selectedAccountTitle = $accountTitles->firstWhere('id', $selectedAccountTitleId);
            }

            if ($selectedAccountTitle !== null) {
                $subAccountTitles = $this->getSelectableSubAccountTitles((int) $selectedAccountTitle->id);
        ...
        return view('cash_bank_ledgers.index', [
            'ledgerType' => $ledgerType,
            'pageTitle' => $ledgerType === 'cash' ? '現金出納帳' : '預金出納帳',
            'debitLabel' => $ledgerType === 'cash' ? '入金' : '預入',
            'creditLabel' => $ledgerType === 'cash' ? '出金' : '引出',
            'routeName' => $ledgerType === 'cash' ? 'cash-ledgers.index' : 'bank-ledgers.index',
            'books' => $books,
    ...
    private function getSelectableAccountTitles(int $bookId): Collection
    {
    ...
    private function getSelectableSubAccountTitles(int $accountTitleId): Collection
    {
    ...
    private function guessDefaultAccountTitle(Collection $accountTitles, string $ledgerType): ?AccountTitle
    {
        if ($accountTitles->isEmpty()) {
            return null;
        }

        $keywords = $ledgerType === 'cash'
            ? ['現金']
            : ['預金', '普通', '当座'];

        foreach ($keywords as $keyword) {
            $matched = $accountTitles->first(function (AccountTitle $accountTitle) use ($keyword): bool {
                return mb_strpos($accountTitle->name, $keyword) !== false
                    || mb_strpos((string) $accountTitle->account_code, $keyword) !== false;
            });
    ...
    private function calculateOpeningBalance(
        ...
        if ($subAccountTitleId !== null) {
            $openingQuery->where('jel.sub_account_title_id', $subAccountTitleId);
        }
    ...
    private function buildLedgerRows(
        ...
        if ($subAccountTitleId !== null) {
            $linesQuery->where('journal_entry_lines.sub_account_title_id', $subAccountTitleId);
        }
